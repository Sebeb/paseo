...
Codex app-server treats per-thread `config.mcp_servers` as the full MCP server set for that thread. When Paseo injects runtime MCP into Codex, the Codex provider must first read the app-server's loaded `config/read.config.mcp_servers` and merge the explicit runtime servers over it; otherwise injected `paseo` MCP replaces the user's normal Codex MCP servers. Normalize known scalar fields before sending the merged config back to app-server: per-thread config validation is stricter than the loaded config echo, so values such as string timeout fields must become numbers or be omitted.
