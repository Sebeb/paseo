...
Anchor commit: d72c6eddf57a1c3d83b7444be0ee72a2b5123449 — fix(codex): normalize inherited MCP server config
...
**Tests added (`codex-app-server-agent.test.ts`):** `"merges configured Codex MCP servers when injecting runtime MCP servers"` — sets up a fake Codex app-server whose `config/read` returns two configured servers: `browser` (with `startup_timeout_sec: "3.5"` and `tool_timeout_sec: "12"`, both strings) and `godot` (with `tool_timeout_sec: ""`, an invalid blank string). Creates a `CodexAppServerAgentSession` with a `paseo` HTTP MCP server injected via `config.mcpServers`, calls `session.startTurn(...)`, and asserts the resulting `thread/start` params' `config.mcp_servers` contains: `browser` with `startup_timeout_sec: 3.5` and `tool_timeout_sec: 12` coerced to numbers (other fields like `command`/`args`/`env` untouched), `godot` with the invalid `tool_timeout_sec` field dropped entirely, and `paseo` present with its `url`/`http_headers` from the injected `McpServerConfig`.
