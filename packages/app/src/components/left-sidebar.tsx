...
export const LeftSidebar = memo(function LeftSidebar({
  ...
  const activeWorkspaceSelectionKeyValue = activeWorkspaceSelection
    ? `${activeWorkspaceSelection.serverId}:${activeWorkspaceSelection.workspaceId}`
    : "";
  const syncedActiveWorkspaceSelectionKeyRef = useRef("");

  useEffect(() => {
    if (!singleProjectViewEnabled || !activeWorkspaceSelectionKeyValue || !activeSingleProject) {
      return;
    }
    if (syncedActiveWorkspaceSelectionKeyRef.current === activeWorkspaceSelectionKeyValue) {
      return;
    }
    syncedActiveWorkspaceSelectionKeyRef.current = activeWorkspaceSelectionKeyValue;
    if (activeSingleProject.projectKey !== singleProjectViewProjectKey) {
      setSingleProjectViewProjectKey(activeSingleProject.projectKey);
    }
  }, [
    activeSingleProject,
    activeWorkspaceSelectionKeyValue,
      ...
      <GestureDetector gesture={closeGesture} touchAction="pan-y">
        <Animated.View style={mobileSidebarStyle} pointerEvents="auto">
          <View style={styles.sidebarContent} pointerEvents="auto">
            <WorkspacesSectionHeader serverId={activeServerId} title={headerProjectName} />
            <Pressable
...
function WorkspacesSectionHeader({
  ...
  const titleStyle = useMemo(
    () => [styles.workspacesSectionTitle, title && styles.workspacesSectionProjectTitle],
    [title],
  );
